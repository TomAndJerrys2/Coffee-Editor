// Unix Text Editor by Kyle Brady @ 2025
#include "Editor.h"
...
char *C_MACRO_keywords[] = {"#". "define", "ifndef", "if", "else", "endif",
							"include", "undef", NULL}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (KEDITOR_TAB_STOP - 1) - (cur_rx % KEDITOR_TAB_STOP);
    cur_rx++;
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs * (KEDITOR_TAB_STOP - 1) + 1);

  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % KEDITOR_TAB_STOP != 0)
        row->render[idx++] = ' ';
    } else {
      ...
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen =
            snprintf(welcome, sizeof(welcome),
                     "[Kyle's Text Editor] -- Version %s", KEDITOR_VERSION);
